...
<Info>
  Available payment methods depend on your MercadoPago account configuration,
  supported countries, and enabled payment types.
</Info>
...
<ResponseField name="response" type="mixed">
  ...
  <Expandable title="Response structure">
    ...
    <ResponseField name="payment_type_id" type="string">
      Payment type: `credit_card`, `debit_card`, `prepaid_card`, `ticket`,
      `bank_transfer`, etc.
    </ResponseField>
    ...
    <ResponseField name="deferred_capture" type="string">
      Whether the method supports deferred capture: `supported`, `unsupported`,
      or `does_not_apply`
    </ResponseField>

    <ResponseField name="settings" type="array">
      Configuration settings: - `security_code` (object): CVV requirements -
      `card_number` (object): Card number validation - `bin` (object): BIN
      detection settings
    </ResponseField>
...
<Note>
  Available payment methods vary by country and account configuration. Always
  fetch methods dynamically rather than hardcoding IDs.
</Note>
...
<ResponseField name="MercadoPagoException" type="exception">
  Thrown by the MercadoPago SDK for API errors: - Invalid credentials - Network
  errors - API unavailability
</ResponseField>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="link"
    href="https://www.mercadopago.com/developers/en/reference/payment_methods/_payment_methods/get"
  >
