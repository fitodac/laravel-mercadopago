...
<Info>
  Test users are only for sandbox/testing environments. They provide test
  credentials and test payment methods for development and QA.
</Info>
...
<ParamField path="sdkHttpClient" type="SdkHttpClient" required>
  HTTP client for making direct API calls to MercadoPago. Automatically injected
  by Laravel's service container.
</ParamField>

<Note>
  Unlike other services, `TestUserService` uses `SdkHttpClient` instead of
  `MercadoPagoClientFactory` because the MercadoPago SDK doesn't provide a
  dedicated test user client.
</Note>
...
<ParamField path="payload" type="array" required>
  ...
  <Expandable title="Payload structure">
    <ParamField path="site_id" type="string" required>
      MercadoPago site identifier for the country: - `MLA` - Argentina - `MLB` -
      Brazil - `MLM` - Mexico - `MLC` - Chile - `MLU` - Uruguay - `MCO` -
      Colombia - `MPE` - Peru
    </ParamField>
  </Expandable>
</ParamField>
...
<Note>
  Always create test users for the same country/site as your production
  MercadoPago account.
</Note>
...
<Note>
  Test card numbers vary by country. Check [MercadoPago's test cards
  documentation](https://www.mercadopago.com/developers/en/docs/checkout-api/integration-test/test-cards)
...
<Warning>
  **Never use test users or test credentials in production environments.** Test
  users only work in MercadoPago's sandbox and will fail in production.
</Warning>
...
<ResponseField name="MercadoPagoException" type="exception">
  Thrown by the MercadoPago API for errors: - Invalid site ID - Invalid
  credentials - API rate limits - Network errors
</ResponseField>

<ResponseField name="MercadoPagoConfigurationException" type="exception">
  Thrown when: - MercadoPago SDK is not properly configured - HTTP client cannot
  be initialized
</ResponseField>
...
<CardGroup cols={2}>
  ...
  <Card
    title="MercadoPago Test Users Documentation"
    icon="link"
    href="https://www.mercadopago.com/developers/en/docs/checkout-api/integration-test/create-test-users"
  >
