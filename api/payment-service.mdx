...
<ParamField path="payload" type="array" required>
  ...
  <Expandable title="Payload structure">
    ...
    <ParamField path="payer" type="array" required>
      Payer information: - `email` (string, required): Payer's email address -
      `identification` (array, optional): ID document details - `type` (string):
      Document type (e.g., "DNI", "CPF") - `number` (string): Document number
    </ParamField>
...
<ResponseField name="response" type="mixed">
  ...
  <Expandable title="Response structure">
    ...
    <ResponseField name="status" type="string">
      Payment status: `approved`, `pending`, `in_process`, `rejected`,
      `cancelled`, `refunded`, or `charged_back`
    </ResponseField>

    <ResponseField name="status_detail" type="string">
      Detailed status reason (e.g., "accredited",
...
<ResponseField name="response" type="mixed">
  MercadoPago payment object with the same structure as the `create()` method
  response. Contains current payment status and all payment details.
</ResponseField>
...
<ResponseField name="MercadoPagoException" type="exception">
  ...
  payment data - Payment not found (for `get()` method) - Network errors
</ResponseField>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="list"
    href="/api/payment-method-service"
  >
...
<CardGroup cols={2}>
  <Card
    title="Processing Payments Guide"
    icon="book"
  ...
  <Card
    ...
    icon="link"
    href="https://www.mercadopago.com/developers/en/reference/payments/_payments/post"
  >
