...
<Note>
  Customer cards require an existing customer. Use
  [CustomerService](/api/customer-service) to create customers first.
</Note>
...
<Info>
  The `token` parameter contains all card information (number, CVV, expiration,
  holder name) securely tokenized by MercadoPago's frontend SDK. Never send raw
  card data to your server.
</Info>
...
<ResponseField name="response" type="mixed">
  ...
  <Expandable title="Response structure">
    ...
    <ResponseField name="cardholder" type="object">
      Cardholder information: - `name` (string): Name on card - `identification`
      (object): ID document
    </ResponseField>

    <ResponseField name="payment_method" type="object">
      Payment method details: - `id` (string): Payment method ID (e.g., "visa")
      - `name` (string): Payment method name - `payment_type_id` (string): Type
      (e.g., "credit_card")
    </ResponseField>
  </Expandable>
</ResponseField>
...
<Warning>
  **Never store or transmit raw card data (numbers, CVV, expiration dates)
  through your server.** Always use MercadoPago.js to tokenize cards client-side
...
<ResponseField name="MercadoPagoException" type="exception">
  ...
  card token - Customer not found - Card not found (for `delete()` method) -
  Duplicate card - Network errors
</ResponseField>
...
<CardGroup cols={2}>
  <Card
    title="Managing Customers Guide"
    icon="book"
    href="/guides/managing-customers"
  >
  ...
  <Card
    ...
    icon="link"
    href="https://www.mercadopago.com/developers/en/reference/cards/_customers_customer_id_cards/post"
  >
