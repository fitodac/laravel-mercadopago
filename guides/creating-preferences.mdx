...
'items._.title' => ['required', 'string'],
'items._.quantity' => ['required', 'integer', 'min:1'],
...
<Note>
  ...
  `src/Http/Requests/CreatePreferenceRequest.php`
</Note>
...
<Warning>
  Back URLs are for user experience only. Never rely on them for payment
  confirmation. Always use webhooks for server-side processing.
</Warning>
...
<Tip>
  ...
  `/api/mercadopago/webhooks`. You can use this URL or create your own custom
  endpoint.
</Tip>
