...
<Warning>
  Refunds cannot be reversed once processed. Always verify payment details
...
<Step title="Funds returned">
  Money returns to customer's account (may take 3-30 days depending on payment
  method)
</Step>
...
<Warning>
  Verify payment status and amount before processing refunds to prevent errors
  and disputes.
</Warning>
...
<Tip>
  Use test credentials and test payments to verify refund functionality before
