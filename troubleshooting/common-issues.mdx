...
<Warning>
  In production (`APP_ENV=production`), demo routes are **always disabled**
  regardless of the `ENABLE_DEMO_ROUTES` setting. This is a security feature.
</Warning>
...
<Note>
  During local development, you can test webhooks without a secret. The package
  will set `validated: false` but still process the payload.
</Note>
