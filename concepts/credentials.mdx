...
<Warning>
  The access token provides full access to your Mercado Pago account. Never
...
<Info>
  The public key can be safely exposed in client-side code. It only allows card
  tokenization, not API operations.
</Info>
...
<Tip>
  Always configure webhook secrets in production to prevent unauthorized webhook
  requests.
</Tip>
...
<Note>
  SDK configuration happens automatically on the first service call. You don't
  need to manually configure the SDK.
</Note>
...
<Warning>
  In multi-tenant scenarios, ensure services are registered as scoped or
  transient, not singletons, to prevent credential leakage between tenants.
</Warning>
