...
<Note>
  The package focuses on encapsulating communication with the Mercado Pago SDK
  and exposing a reusable API foundation. It does **not** include migrations,
  Blade views, frontend components, payment persistence, or pre-configured
  queues.
</Note>
...
<Steps>
  ...
  <Step title="Laravel 12">
    Compatible with Laravel 12 framework components: - `illuminate/contracts
    ^12.0` - `illuminate/http ^12.0` - `illuminate/routing ^12.0` -
...
<Warning>
  The demo routes are support tools, not a complete business layer. For
  production use, inject the services into your own controllers with proper
